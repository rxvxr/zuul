class Player 
{
    ...
    public Player()
    {
        CurrentRoom = null;
        health = 100;
        backpack = new Inventory(25); // Paramater geeft maxWeight mee (25)
    }
    ...
    {
        if (backpack.RetrieveItems().ContainsKey(itemName))
        {
            if (itemName.ToLower() == "medkit")
            {
                ...
                backpack.RetrieveItems().Remove(itemName);
